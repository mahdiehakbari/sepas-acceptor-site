...
import { months, payoutTypeMap, years } from './constants';
...
const PerformanceReport = () => {
  ...
  const payoutType = overAllData?.lastContractType?.payoutType;
  const payoutLabel = payoutType ? payoutTypeMap[payoutType] : null;

  const contractStatusText = payoutLabel ? `فعال (${payoutLabel})` : 'نامشخص';

  const contractStatusColor = payoutLabel ? 'text-[#237D24]' : 'text-[#F4A400]';
      ...
      <div className='mt-16 mb-6 md:mx-16 bg-[#E9F4FC] p-4 rounded-2xl'>
        <div className='md:flex items-center justify-between mb-2'>
          ...
          <div className='flex justify-between md:justify-start items-center gap-1'>
            ...
            <h3 className={`font-medium text-[16px] ${contractStatusColor}`}>
              {contractStatusText}
            </h3>
          </div>
        </div>
